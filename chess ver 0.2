...
void PlayChess()
{
    ...
    if( WhoseTurn % 2)
    {
        printf("Player 1's turn,enter the coordinate(row,column) of chess piece(ex.1 2)\n");
        scanf("%d %d",&i,&j);
        while(chessboard[i][j] != 0 || i > N || j > N)
        {
            printf("occupied position!Enter another position!\n");
            scanf("%d %d",&i,&j);
        }
        chessboard[i][j] = 1;
    }
    ...
    {
        printf("Player 2's turn,enter the coordinate of chess piece(ex.1 2)\n");
        scanf("%d %d",&i,&j);
        while(chessboard[i][j] != 0 || i > N || j > N)
        {
            printf("occupied position!Enter another position!\n");
